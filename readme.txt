...
= 1.4.110 =
Allows removal of plugins whose findings rest on several malicious behaviours at once, while keeping the protection that prevents a single suspicious behaviour from removing a plugin. A recovered backdoor created a hidden administrator account, concealed it from the Users screen, concealed itself from the Plugins screen and recreated the account after deletion; it was detected correctly and then refused removal by the package-scope rule, so it remained installed. That rule was introduced after this plugin quarantined WP-Optimize on a single behavioural signal and stopped a site from booting, and it had now been applied three times to findings where it made no sense. The distinction is between one suspicious behaviour, which legitimate software can exhibit, and a conjunction no legitimate software presents. The unauthenticated sign-in endpoint is deliberately excluded from the new tier, since managed hosts ship exactly that pattern. Core protection, operator Safe decisions and the circuit breaker all continue to outrank it.

= 1.4.109 =
Closes the gap that let a banned plugin keep reappearing. The site-policy ban covered uploading, activating and listing a banned plugin, and removed one found installed, but did not cover installing one from the WordPress.org directory, so a banned plugin could be installed freely and remained until the next scan removed it. Installation is now refused before the package is unpacked, covering both directory installs and uploaded archives, and banned plugins are labelled in the directory search so the attempt is discouraged beforehand. The plugin roster now records the account and address behind each installation, counts repeat installations and raises a finding at the third, and a refused install names the account in both the log and the administrator notification, since an account able to install banned software should be treated as compromised if the attempt was not the operator's.

= 1.4.108 =
Blocks the infrastructure behind malware rather than only removing the file. When a doorway script or backdoor is confirmed, the scanner recovers the command-and-control hosts it contacts by applying the same reversible decodings the sample uses, including rot13 over URL-encoding, which is how the recovered wp-slgnup sample concealed its four hosts so that no domain appeared anywhere in the file. Recovered hosts are recorded and the outbound guard refuses every request to them, which outlasts deleting any particular file since a replacement dropper calls the same infrastructure. Entries expire after ninety days, and the site's own host and the WordPress ecosystem are never eligible. Also blocks the address attributed to a piece of malware on this site for thirty days, where previously it was reported to Akismet and left unblocked locally.

= 1.4.107 =
Fixes a protection rule that shielded malware. Since 1.4.89 every PHP file in the WordPress root was treated as core and refused automatic removal, which protected core and equally protected anything dropped beside it; a recovered doorway script named wp-slgnup.php, one letter from wp-signup.php, was refused removal even against a confirmed signature match. Core is now the fixed set of files WordPress actually ships, plus wp-config.php, and a non-core PHP file in the root is treated as a location where an unexpected executable does not belong. Also adds detection for search-engine cloaking: the recovered sample identifies crawlers and referrers, fetches content from elsewhere and serves it with forged headers while showing visitors the real site, and it carried neither an eval nor an encoded blob, so the control-flow-flattening check scored one tell short of reporting. Cloaking now counts as evidence of intent and authorises removal, since obfuscated files are otherwise held for review and no obfuscator produces cloaking.

= 1.4.106 =
Completes the headless-folder cleanup introduced in 1.4.105. That release quarantined a plugin-shaped folder containing no plugin, named the options it had declared in wp_options, and asked the operator to delete them by hand; since those entries are precisely the part that survives deletion of the folder, the instruction repeated the problem the finding described. The options are now quarantined alongside the folder using the existing option store, so their values remain recoverable and preserved as evidence. The cleanup respects the auto-delete setting: with automatic removal switched off nothing is touched, and the finding names the options and explains that enabling removal would handle them.

= 1.4.105 =
Adds detection for plugin-shaped folders containing no plugin. A recovered sample was packaged with a readme, a licence and a translation template but no file declaring a Plugin Name, so WordPress could not load it; it existed only to carry two high-entropy data files and to declare two options. Because every other check reads PHP and the folder contained almost none, no existing check detected it. Such a folder indicates either a loader already removed with its payload left behind, or a payload staged ahead of its loader. The check reads the folder's shape rather than its contents and is kept narrow, additionally requiring an opaque data file or an uninstall script declaring options, so library and asset folders are unaffected. The finding names the options, since they survive deletion of the folder. A new policy rule permits removal on the basis that a folder declaring no Plugin Name is not installed software, leaving the protection for real plugin packages unchanged.
...
= 1.4.99 =
Fixes a bypass of the unauthenticated-file-manager check and adds detection for a third eval-avoiding execution technique, both found in one sample: a trojanized copy of the genuine bbPress plugin carrying four unrelated malware tools. The bypass: a full file manager detected WordPress with `defined('ABSPATH') || defined('WPINC')` used only as a feature flag, which the existing check mistook for the legitimate "refuse to run standalone" guard and skipped the file entirely; the guard test is now shape-based (negated and followed by exit/die/return) rather than presence-based, verified against this plugin's own files and a stock bbPress copy to confirm no false positives. The new technique: a custom stream-wrapper protocol whose handler decodes its own path and returns the result as file content, then included through that protocol - avoiding eval(), tmpfile(), and self-reads alike. A plain unauthenticated file manager and a 1MB chr()-obfuscated payload in the same sample were already caught by existing checks; nothing changed for those.

= 1.4.98 =
Adds detection for an eval()-avoiding loader found inside a trojanized copy of a legitimate WordPress.org plugin: a base64 blob decoded and executed by writing it into an anonymous temp file wrapped as PHP and including that file, never calling eval(), assert(), or create_function(), which let it slip past the existing decoder-chain check. Matches on that structural shape (tmpfile + fwrite of a <?php wrapper + stream_get_meta_data + include) rather than any signature string, so re-encoding the payload doesn't evade it. Also closes a second gap the same sample exposed: one copy of the payload was saved with a .txt extension, which every existing check gated on PHP-file-extension filtering skipped outright: this check additionally reads the first bytes of a short list of non-PHP extensions and treats a leading <?php tag as PHP regardless of the filename.

= 1.4.97 =
Adds FileOrganizer (Softaculous) to the site-policy banned-plugins list. It is legitimate software, not malware - the same risk class as WP File Manager: another elFinder-based full-filesystem file manager, just a different vendor. Refused on upload and activation (also catches the fileorganizer-pro add-on via substring match), removed if already installed, and flagged as high-risk even with the banned-plugins feature switched off. Three plugins now ship banned by default: WP File Manager, FileOrganizer, and FileBird.

= 1.4.96 =
Adds detection and automatic removal for four confirmed second-stage samples recovered together as a kit: a standalone file copy of the SSO login-bypass loader (complementing the 1.4.95 runtime guard, which disarms the endpoint but never touches the file), a Basic-Auth-gated unauthenticated file manager ("WordPress Test Shell"), a password-gated arbitrary file-write editor that maintains a whitelist of plausible-sounding filenames to write PHP back into, and an XMRig cryptominer loader that downloads and launches a detached mining binary using a hardcoded wallet address. All four are matched wherever the existing malware-signature scan already runs - plugins, themes, mu-plugins, uploads, language directories, writable cache/fonts/upgrade directories, and the site root - and removed automatically outside the root, the same as other confirmed signature matches. Two of the four are also added to the known-backdoor-filename list so a copy dropped directly at the site root is auto-removed there too; the third (policies.php) is left as a manual-review finding at the root only, since that filename alone is too plausible to safely auto-delete without the location context the other checks already provide.

= 1.4.95 =
Adds detection and blocking for unauthenticated administrator sign-in endpoints, such as the SSO loader managed hosts install in mu-plugins, which grants an administrator session to any caller presenting the right token with no password and no second factor. Detection is structural - a nopriv AJAX action or always-true REST permission callback in the same file as a call establishing a session - so it catches an attacker's copy as readily as the genuine article, and it reports rather than removing, since the file belongs to the host and will be redeployed. Blocking is a runtime guard rather than a deletion for that reason, and because the file is not the bypass: the token in wp_options is a standing administrator credential that survives every password reset. The guard removes the endpoint registration on every request and clears the token. Off by default, since enabling it stops the host's one-click dashboard login.

= 1.4.94 =
Fixes confirmed malware being reported rather than removed. The remediation policy classifies detectors that match known-malicious content as evidence and everything else as inference, but its list named three narrow signature variants and omitted the plain "Malware signature in PHP file" type used by the main signature scan, so the strongest evidence the plugin produces was treated as a guess and left in place. The missing types are added, and a new completeness gate now derives the list mechanically from the types the scanner can actually emit and fails if any signature or known-malware type is unclassified - which immediately found two further omissions. Conservative behaviour elsewhere is unchanged: core is never removed automatically even on a signature match, behavioural findings inside installed software are still reported rather than removed, and operator Safe decisions still override everything.

= 1.4.93 =
Adds bulk actions to the scan findings list, following an audit of every list and control in the plugin for the problem previously fixed in the hostile-IP list. Findings with a file target now carry checkboxes, with select-all and two bulk operations: mark selected Safe, or quarantine selected. Bulk quarantine is not a shortcut past the remediation policy - every selected target passes through the same gate the scanner obeys, so Safe targets, WordPress core files and files inside installed plugins and themes are skipped and counted rather than removed, since an operator clearing a long list is not reviewing each row. The audit found quarantine already had bulk operations and the remaining lists short enough by nature not to need them; those findings are recorded rather than changed speculatively.

= 1.4.92 =
Adds bulk reporting to the hostile-IP block list. The list routinely exceeds forty rows and every row carried its own report button, which made a task the plugin encourages effectively unusable. A single control above the table now reports every unreported address, showing the count and capped at fifty per click. All existing safeguards are preserved - ranges are never submitted, already-reported addresses are skipped, and each submission goes through the same guarded path - and one is tightened: the shared-infrastructure guard, which the per-row button exempts on the grounds that a human clicking one address has reviewed it, is reapplied for bulk submissions, since nobody reviews forty rows individually. Skipped addresses are counted and explained rather than silently dropped.
...
= 1.4.86 =
Adds detection for plugins that store their executable payload in a WordPress option rather than in their own files, writing it back whenever the option is missing. Three new samples of the random-suffix packed family use this, and while all three were already detected by the split-identifier check, the existing finding did not tell the operator that deleting the plugin folder leaves the payload behind in wp_options for the next loader to find. The new check requires a hardcoded encoded blob, an update_option writing it and a get_option reading it back, names the option in the finding, and quarantines it reversibly alongside the folder. Also fixes a defect in that check found during development, where one throwing file aborted the entire directory walk and caused every later file to go unreported.

= 1.4.85 =
Extends Akismet reporting to two cases that were going unused. Confirmed injected gambling and SEO-spam comments carry the sender's address in comment_author_IP, which is Akismet's canonical case, and those authors are now reported (capped per scan). Separately, when a scan confirms malware, the plugin looks for an address in its own hostile-IP records that was active when that file was written - the file's modification time against the recorded activity window plus a margin - and reports it with the timing stated. This is deliberately narrow: only addresses already blocked for hostile behaviour are considered, the timing must line up, at most one address is blamed per file, ranges are never submitted, and no timing match means no report rather than a guess. Note that Akismet accepts addresses, not file hashes, so malware itself cannot be submitted; what is reported is the address responsible where evidence exists.

= 1.4.84 =
Repairs the control-flow-flattening check, which read a real packed backdoor as clean. The sample used 121 goto jumps across four lines and sent data to a Telegram bot, but wrote every string in hex and octal escapes, so the two tells that would have proven intent - a readable execution sink and a long encoded payload run - both scored zero, leaving it one tell below the reporting floor. The payload and execution tells now run against escape-resolved text, which this plugin has been able to produce since 1.4.34 but this check never used. Adds a sixth tell for a messaging-bot or webhook endpoint that is hidden in the raw file and only appears once escapes are resolved; a plainly visible webhook is ordinary and is not counted.

= 1.4.83 =
Adds a plugin roster that reports any plugin folder appearing without an installation being recorded through the dashboard or WP-CLI, regardless of whether the plugin's own code is clean. This addresses a real gap: an intruder frequently installs a legitimate, working tool such as a file manager because it draws less attention than a web shell and matches no malware signature, so every content-based check correctly returns clean while the site remains compromised. The presence of an uninstalled plugin is evidence that someone other than the owner could place files on the server, and its folder date approximates when. The first scan adopts existing plugins as a baseline and reports nothing, so upgrading does not produce a wall of false alarms; afterwards, anything new is reported once. Detection only, since a plugin may legitimately have been installed by SFTP.

= 1.4.82 =
Fixes a gap where the encoded-payload-loader check reported backdoors without removing them, leaving a live remote-code loader on disk while describing it accurately in the scan report. That check now requests removal like every other check of comparable severity, through the same quarantine-first path so the action stays reversible and the evidence is preserved. It removes only the offending file, never the surrounding directory, because these loaders are typically planted among legitimate files in wp-content/uploads; loaders that are part of a purpose-built malicious plugin continue to be removed folder and all by the plugin-level checks.

= 1.4.81 =
Adds detection for hidden administrator backdoors: code that creates an administrator account and then conceals it from the Users screen and the REST API, typically recreating it after deletion and writing its credentials in escape codes to defeat searching. Both halves are required before anything is reported - creating an administrator is legitimate, hiding one is not - so genuine user-management tools are unaffected. The finding gives the remediation order explicitly, since deleting the account before removing the code simply restores it. Also repairs two evasions in the encoded-payload check found in a live sample: a payload stored as raw bytes after the closing PHP tag rather than in a quoted string, and an execution sink split apart by comments and prefixed with the error-suppression operator.

= 1.4.80 =
Adds detection for two further techniques from the ongoing campaign: code broken apart by junk comments inserted between tokens (which runs normally but defeats searching, and additionally uses random capitalisation against case-sensitive signatures), and tiny typosquat plugins whose only behaviour is injecting a remote script into every page. The comment-obfuscation check strips comments and compares against the raw text, flagging the case where dangerous calls appear only after stripping - an honest file shows no such gap. Because this family plants files inside genuine unmodified plugins, that finding removes the planted file rather than the surrounding plugin and says the rest of the folder should be verified against the official source. Also repairs the opaque-data-payload check, which analysed one file at a time and so missed a variant that named its data blob in one file and read it in another; evidence is now gathered across the whole plugin.

= 1.4.79 =
Adds detection for a packed fake-plugin family that defeated every existing content check (measured, not assumed - the harness asserts the pre-1.4.79 checks find nothing on the samples). These plugins install under plausible names with a random four-hex suffix and keep their behaviour in opaque data files rather than PHP, so code-reading scans see a trivial clean plugin. Two new structural checks: one flags identifiers split across concatenation to defeat search (requiring a mid-word split, a density threshold, and that joining the pieces reveals a real API name), and one flags a plugin that reads an opaque data blob and either writes PHP out of it or hands it to a decoder or executing sink. One recovered variant self-reconstructs its PHP from an XOR-encrypted data file when the PHP is deleted, so removal targets the whole folder. Ordinary concatenation, binary font assets, and binary caches that are read but never executed are all left alone.

= 1.4.78 =
Fixes two defects in the encoded-payload-loader check, found by running a live sample against the shipped code. A planted loader inside a legitimate Filester install - seven directory levels deep in the plugin's own upload area, chaining eval over htmlspecialchars_decode, base64_decode and urldecode - returned zero findings. The directory walk stopped at depth 5, and only one of the loader's four decode layers was on the list the check counts, leaving it under the two-layer floor. The payload scan now walks to depth 10 via a named constant, and transport decoders (urldecode, rawurldecode, html_entity_decode, htmlspecialchars_decode, stripslashes) count as layers. Also confirms two other uploads needed no new work: a repeat of the .htaccess-injected WP File Manager copy already covered in 1.4.76, and an empty directory left behind by a deleted ClickFix-family plugin.

= 1.4.77 =
Adds detection for three malicious artefacts recovered from a compromised site: the VeyronHacklink backlink-injection family (Turkish SEO hacklink kit that pulls links from a master server, injects them into the footer, and self-promotes into mu-plugins to survive plugin deletion), a fake "XDav Tracker" plugin that hides itself from the Plugins list and serves obfuscated script only to non-administrators, and two unauthenticated PHP file managers buried inside a homoglyph typosquat of WPForms Lite. Beyond named signatures, adds a structural check for self-concealing plugins: any plugin that removes its own entry from the Plugins list, or withholds its front-end output from administrators while serving it to visitors. Neither behaviour has a legitimate use, and the check survives a variant rewriting its payload. Verified against the real samples.

= 1.4.76 =
Detects web-shell persistence hidden in .htaccess, a compromise class where no PHP file is modified and code-based scanning returns clean. Flags any .htaccess that denies PHP while allowlisting PHP filenames WordPress does not ship - recovered from a live case of 188 identical files injected through a plugin tree, allowlisting about.php, radio.php, content.php and lock360.php. Detection is structural, so a variant that renames its shells is still caught, and findings are grouped into one incident rather than one per file. Ordinary hardening (plain Deny from all, WordPress-only allowlists, the standard rewrite block) is not affected. Adds lock360.php and radio.php as named signatures under a new family label.

= 1.4.75 =
Completes the site-policy banned-plugins feature: a banned plugin found installed on disk is now quarantined and removed, not merely blocked from upload and activation. Prompted by a live sample of WP File Manager whose own files were byte-identical to the clean release but which carried 188 attacker-written .htaccess files through its folder tree, allowlisting a fixed set of webshell filenames - a persistence mechanism signature scanning alone would miss. Removal is quarantine-first and reversible, honours the auto-delete setting, is gated on the banned-plugins switch, matches by exact folder name so similarly-named plugins are untouched, and is recorded as a site-policy decision rather than a malware detection, since these plugins are legitimate software.
